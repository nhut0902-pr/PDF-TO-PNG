...
export const TikTokDownloader: React.FC<Props> = ({ language }) => {
                  ...
                  {history.map((item) => (
                    ...
                    >
                      ...
                      <div className="ml-3 flex-grow min-w-0">
                        <h5 className="text-[11px] font-black text-slate-900 dark:text-white truncate uppercase tracking-tight">@{item.authorNickname}</h5>
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate mt-0.5 font-medium italic">{item.title}</p>
                        <div className="flex items-center mt-1 space-x-2">
                           <span className="text-[8px] font-black text-slate-300 dark:text-slate-600 uppercase">{formatTime(item.timestamp)}</span>
                        </div>
                      </div>
                      <button onClick={(e) => removeFromHistory(item.id, e)} className="ml-2 p-2 text-slate-200 dark:text-slate-700 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all">
